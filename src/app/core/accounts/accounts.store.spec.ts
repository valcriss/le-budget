...
describe('AccountsStore', () => {
  ...
  it('skips loading when data already present and not forced', async () => {
    (store as any).accountsSignal.set([toAccount({ id: 'seed' })]);
  ...
  it('updates accounts and resorts list', async () => {
    (store as any).accountsSignal.set([
  ...
  it('refreshes existing accounts and removes missing ones', async () => {
    (store as any).accountsSignal.set([toAccount({ id: 'acc-1' })]);
  ...
  it('normalizes helper outputs and maps fallback errors', () => {
    ...
    expect(anyStore.normalizeCurrency('usd')).toBe('USD');
    expect(anyStore.normalizeCurrency(null)).toBe('EUR');
    expect(anyStore.toAccountType('credit_card')).toBe('CREDIT_CARD');
    expect(anyStore.toAccountType('custom')).toBe('OTHER');
    expect(anyStore.toNumber('12')).toBe(12);
    expect(anyStore.toNumber('oops')).toBe(0);
    expect(anyStore.toIsoString('2024-01-01T00:00:00Z')).toBe('2024-01-01T00:00:00.000Z');
    expect(anyStore.toIsoString('invalid')).toBeNull();
    ...
    expect(anyStore.mapError(error, 'fallback')).toBe('Invalid');
    expect(anyStore.extractBackendMessage({ error: 'Error message' })).toBe('Error message');
  });
});
